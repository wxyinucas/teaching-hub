...
- 历史只讲三步：
  - 文字终端先于图形界面出现；
  - 鼠标和窗口降低了发现与操作门槛；
  - 文字命令因为便于保存、复制、远程执行和自动化而一直保留下来。
- 图形界面：
  - 容易发现功能、直接看到空间关系，适合探索和一次性操作；
  - 大量重复操作不容易准确复现。
- 命令行：
  - 动作和对象写得明确，容易记录、重放和组合；
  - 代价是必须看清当前目录、路径和报错。
- 明确：
  - 这不是“先进／落后”的比较，而是操作同一个系统时两种不同的侧重。
...
- 第一次见到命令时顺手拆名字：
  - `mkdir` 来自 **make directory**，即“创建目录”；`-p` 可记作 **parents**，需要时连同上级目录一起创建，目标已存在也不报错；
  - `cd` 来自 **change directory**，即“切换目录”；
  - `code` 是 VS Code 提供的命令名，不是缩写；`.` 表示当前目录。
...
- 指出：
  - `uname` 没有需要死记的官方展开，可借 **Unix name** 联想它用于报告系统信息；这里的 `-s` 只显示 kernel name，在 WSL 中通常是 `Linux`；
  - `pwd` 来自 **print working directory**，回答“我在哪里”；
  - `ls` 来自 **list**，回答“这里有什么”；
...
- 明确：
  ...
  - “VS Code 打开了某个文件夹”与“终端当前位于某个目录”需要分别核对。

- **转场：**
  - `ls` 已经能够调用，`tree` 还不一定存在。
  - 下一步亲手给系统增加一个新命令。
...
- `command` 是 Bash 的内建命令；这里的 `-v` 用来询问“这个名字会被解析成什么”。不要把每个单字母选项都硬凑成英文全称，应以命令帮助为准。
- `tree` 就是英文“树”，不是缩写；它用树状形式显示目录层级。
...
- `sudo` 常被记作 **superuser do**；
  - 更准确的行为是“以指定用户身份执行”，未指定时通常以管理员 `root` 执行。
  - 这里借名字帮助记忆，不把它误解成只能切换到 root。
- `apt` 来自 **Advanced Package Tool**；
  - `update` 是更新可用软件清单的 subcommand；
  - `install` 是安装指定软件的 subcommand。
- 说明：
  - 输入 `sudo` 密码时终端不会显示字符；
  - 输入 WSL 用户密码后按 Enter。
  - 安装询问是否继续时，确认即将安装的是 `tree` 再选择继续。
- 安装后重新运行：
...
- 本段出口：
  ...
  - Explorer、`ls` 与 `tree` 显示的结构一致，终端能够读出文件内容。

- 本段要证明：
  - 终端可以建立目录与空文件，
  - VS Code 编辑器可以写入同一个文件，
  - 终端随后能够读到变化；Explorer、`ls` 与 `tree` 只是同一状态的不同视图。

- 本段命令工具箱：
  - `cd`、`pwd`、`mkdir`、`touch`、`cat`、`ls`、`tree`；
  - 按现场节奏输入，不要求逐行照读台本。
- 讲解只抓三个点：
  - `mkdir` 是 **make directory**；
  - `touch` 不是缩写，本义是“触碰”文件并更新时间戳，文件不存在时也会创建空文件；
  - `cat` 来自 **concatenate**，本义是连接文件内容，这里用它把一个文件的内容输出到终端。
- 路径只抓四个符号：
  - `~` 是 home，
  - `.` 是当前目录，
  - `..` 是上一级目录，
...
- 本段出口：
  - 达到本课开头给出的完整目录树，
  - 并能说明副本如何从 `inbox` 到达平级的 `archive`。
- 本段要证明：
  - `cp` 来自 **copy**，保留源文件并产生副本；
  - `mv` 来自 **move**，改变副本所在的路径；
  - 从 `inbox` 出发，`../archive/` 指向一个平级目录。
- 本段命令工具箱：
  - `cp <源路径> <目标路径>`、`mv <源路径> <目标路径>`、`mkdir`、`cd`、`pwd`、`tree`；
  - 命令顺序由上面的目标状态决定。
- 核验只看三件事：
  - 源文件是否仍在、
  - 目标文件是否到达 `archive`、
...
- 清理出口：
  - `note.txt`、`note-copy.txt` 与 `archive` 已删除；
  - 空的 `~/course/cli-lab/inbox` 被保留，`pwd` 显示已经回到 `~/course`。
- 本段命令工具箱：
  - `rm <文件>`、`rmdir <空目录>`、`cd`、`pwd`、`tree`；
  - 先核对路径，再决定删除对象。
- `rm` 来自 **remove**；`rmdir` 来自 **remove directory**。
  - 名字能帮助记忆动作，但不能代替删除前的路径核对。
- 清理原则：
  - `rm` 只指向本次实验创建且已经核对过的文件，
  - `rmdir` 只处理确认已经为空的目录；
  - 每次删除后都从 Explorer 或 `tree` 验证状态。
- 明确：
  - `rm` 不经过 Windows 回收站；
  - 本节只删除自己刚刚创建并已经核对路径的实验文件，不使用 `rm -r` 或 `rm -rf`。
- 收束：
  - GUI 中的复制、拖动和删除，与 `cp`、`mv`、`rm` 改变的是同一份文件系统状态；
  - 命令行的区别是把动作与路径写成了可以复述的文字。
...
- 最后只收四句话：
  ...
  2. 当前目录决定相对路径从哪里开始解释；
  3. 一条命令要分清 command、可能的 subcommand、options 与 positional arguments；
...
- `code`：调用 VS Code 的命令行入口；
  - `-n`（`--new-window`）：在新窗口打开；`~/course/cli-lab`：作为位置参数传入的目标目录，其中 `~` 代表当前用户的 home。
  - 这个命令只打开已有目录，不负责创建它。
...
- `vim` 来自 **Vi IMproved**；本课只把它当作终端中的文本编辑器，不展开编辑器流派。
- Vim 只演示本次必需的四步：
  - 按 `i` 进入插入模式，写入 `print("hello world!")`，按 `Esc`，输入 `:wq` 后按 Enter 保存退出；
  - 操作失控时按 `Esc` 后输入 `:q!` 放弃退出并直接求助。
- 运行：
...
- `Python` 是语言名称，不是缩写；名字来自喜剧团体 Monty Python。末尾的 `3` 表示调用 Python 3 系列解释器。
- 出口：
  - 终端打印 `hello world!`，Explorer 中出现 `inbox/some.py`；
  - 此处不讲 Python 语法，只确认“解释器读到文件并执行了一条语句”。
...
- 预期：
  - `python3 some.py` 报告找不到脚本；
  - 显式写出 `inbox/some.py` 后再次打印 `hello world!`。
- 在 Explorer 中指出 `some.py` 始终存在；失败不是“文件消失了”，而是当前目录下没有这条相对路径。
- 不立刻给出抽象定义，只追问：
  - 同一条命令中，为什么 `python3` 仍然有效，而 `some.py` 需要换一种写法？
...
- 先让环境变量从抽象名词变成眼前的对象：它们以 `名称=值` 的形式存在于当前终端会话中。投影时先只列名称，避免意外展示 token 等私密值：
...
- 指出这些名称承担不同职责：
  - `USER` 是当前用户名，
  - `HOME` 是 home 目录，
  - `SHELL` 记录账号的默认 shell（不一定等于此刻正在运行的 shell），
  - `PWD` 是当前工作目录；
  - `PATH` 则是本段真正要追踪的“程序搜索目录表”。
- `PATH` 不是需要逐字展开的缩写，而是名为“path／路径”的环境变量；
  - 大写是 shell 环境变量的常见命名方式。它保存 shell 搜索可执行程序时依次检查的目录。
- 把 `PATH` 中由冒号分隔的目录拆成逐行显示；
  - 只要求学生观察“它不是一个目录，而是一组有顺序的目录”：
...
- “全局／局部”不是对象天生的标签，必须先问“相对于谁、在哪个范围”：
  - 课堂口语里的“全局工具”，准确地说只是当前 shell 能从 `PATH` 找到、因而可在多个项目中调用的工具。
- 以后讲程序变量时也会遇到类似问题：
  - 一个名字在当前位置是否可见。
  - 这里只保留这个共同的观察视角；
  - 文件路径、`PATH`、环境变量和 Python 变量并不是同一套机制。
...
- 先保留机器事实：
  - `command -v python3` 和 `python3 --version` 分别回答“调用的是谁”和“它是什么版本”。

- 提出场景：
  - 两台机器都能运行 `python3`，但一台是 `3.10`，另一台是 `3.12`；
  - 程序还能否保证得到同样结果？

- 今天运行成功，只能证明“当前代码 + 当前 Python + 当前依赖”能够配合工作；
  - 代码即使没有改变，重新安装时得到的运行条件仍可能发生变化。
- 如果不记录版本，几个月后的作者——包括未来的自己——可能已经无法运行今天成功的代码；
  - 锁定版本，是把一组已经解析、验证过的运行条件保存下来。
...
- 在 Explorer 中观察 `.venv`：
  ...
  - `.venv` 是这台机器据此恢复出的结果，可以重新生成。
- 学生主线不执行 `source .venv/bin/activate`；如需比较，教师可以简短展示激活方式。本课程统一用 `uv run` 明确说明“从当前项目环境运行”。
...
- `unset` 是英文“取消设置”，从当前 shell 中移除这个变量；
  - `printenv` 来自 **print environment**，打印环境变量；
  - `pytest` 是 Python 测试工具的命令名，`-q` 来自 **quiet**，减少输出。

- 程序输出应包含项目 `ai-agents-lab`、版本 `0.1.0`、Python `3.12.x`、`project-env: PASS` 与 `course-mode: fixture`；
  - 此时签名仍可保持教师默认值 `teacher`。
- pytest 应全部通过；
  - 它只说明当前写下的规则通过，不代表所有可能的问题都已经被覆盖。
...
- 出口：
  - 程序显示新的公开代号与 `RUNTIME_CHECK=PASS`；
  - 不写姓名、完整学号、邮箱或其他个人信息。
- 提问：
...
- 完整对象 ID 指认一个 commit；
  ...
  - `HEAD` 是一个引用；
  - 在本课的正常分支状态下，它经当前分支解析到最新 commit。
  - `git show HEAD` 和 `git show <sha>` 都可以查看对应历史。
- 只介绍、不执行：
